...
class TestComponentInfo(unittest.TestCase):
    ...
    def test_print_all_default_components_as_markdown(self):
        ...
        print(
            pd.DataFrame(
                zip(d.keys(), map(f, d.values()), map(g, d.values()))
    ...
    def test_print_all_models_as_markdown(self):
        ...
        for lang in nlu.Spellbook.pretrained_models_references.keys():
            for nlu_reference in nlu.Spellbook.pretrained_models_references[
                lang
            ].keys():
                rows.append(
                    (
    ...
    def test_print_all_HC_models_as_markdown(self):
        ...
        for lang in nlu.Spellbook.pretrained_healthcare_model_references.keys():
            for nlu_reference in nlu.Spellbook.pretrained_healthcare_model_references[
                lang
            ].keys():
                rows.append(
                    (
                        lang,
                        nlu_reference,
                        nlu.Spellbook.pretrained_healthcare_model_references[lang][
                            nlu_reference
                        ],
                    )
                )
    ...
    def test_print_all_HC_pipes_as_markdown(self):
        ...
        for lang in nlu.Spellbook.pretrained_healthcare_pipe_references.keys():
            for nlu_reference in nlu.Spellbook.pretrained_healthcare_pipe_references[
                lang
            ].keys():
                rows.append(
                    (
                        lang,
                        nlu_reference,
                        nlu.Spellbook.pretrained_healthcare_pipe_references[lang][
                            nlu_reference
                        ],
                    )
                )
    ...
    def test_get_count_for_every_component_type(self):
        ...
        for lang in nlu.Spellbook.pretrained_models_references:
            for nlu_ref, nlp_ref in nlu.Spellbook.pretrained_models_references[
                lang
            ].items():
                ...
                if c_type not in component_counts:
                    component_counts[c_type] = []
                if (lang + nlp_ref) not in component_counts[c_type]:
                    component_counts[c_type].append(lang + nlp_ref)
