...
from nlu import *
from tests.test_utils import get_sample_pdf
...
class TestNameSpace(unittest.TestCase):
    ...
    def test_sentiment_twitter_out(self):
        ...
        res = nlu.load("en.sentiment.imdb", verbose=True).predict(
    ...
    def test_output_levels(self):
        print("token test")
        df = nlu.load("sentiment", verbose=True).predict(
            "What a wonderful day!", output_level="token"
        )
        print(df)
        print("document test")
        df = nlu.load("sentiment", verbose=True).predict(
            "What a wonderful day!", output_level="document"
        )
        print(df)

        print("sentence test")
        df = nlu.load("sentiment", verbose=True).predict(
            "What a wonderful day!", output_level="sentence"
        )
        print(df)

        print("chunk test")
        df = nlu.load("sentiment", verbose=True).predict(
            "I like peanut butter and jelly!", output_level="chunk"
        )
        print(df)

    def test_ner_multilingual(self):
        df = nlu.load("ner", verbose=True).predict(
            "New York is a great place and America aswell"
        )
    ...
    def test_sentence_detector(self):
        df = nlu.load("sentence_detector", verbose=True).predict(
            "What a wonderful day! Tomorrow will be even better!"
        )
    ...
    def test_sentiment_output(self):
        res = nlu.load("sentiment", verbose=True).predict(
    ...
    def test_embed_sentence_bert(self):
        pdf = get_sample_pdf()
        res = nlu.load(
            "en.embed_sentence.biobert.pubmed_base_cased", verbose=True
        ).predict(pdf)
        print(res)
    ...
    def test_26_bert(self):
        res = nlu.load("en.ner.bert", verbose=True).predict(
            "The NLU library is a machine learning library, simmilar to Tensorflow and Keras"
        )
        print(res)
