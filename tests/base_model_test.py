...
def all_annotator_tests():
    return [
        ...
        NluTest(nlu_ref="en.seq2seq.distilbart_cnn_6_6", lang='en', test_group='seq2seq', input_data_type='summarizer',
                library='open_source', pipe_params=[PipeParams('bart_transformer', 'setTask', 'summarize:'),
                                                    PipeParams('bart_transformer', 'setMaxOutputLength', 200)]),
        ...
        NluTest(nlu_ref="en.t5.small", lang='en', test_group='seq2seq', input_data_type='generic',
                library='open_source',
                pipe_params=[PipeParams('t5_transformer', 'setTask', 'translate English to French')]),
        ...
        NluTest(nlu_ref="en.zero_shot.ner_roberta", lang='en', test_group='hc_generation', input_data_type='medical',
                library='healthcare',
                output_levels=["sentence"], pipe_params=[PipeParams('zero_shot_ner', 'setEntityDefinitions', {
                "PROBLEM": [
                    "What is the disease?",
                    "What is his symptom?",
                    "What is her disease?",
        ...
        NluTest(nlu_ref="en.med_ner.clinical en.relation.zeroshot_biobert", lang='en', test_group='hc_generation',
                input_data_type='medical', library='healthcare',
                pipe_params=[PipeParams('zero_shot_relation_extraction', 'setRelationalCategories', {
                    "CURE": ["{{TREATMENT}} cures {{PROBLEM}}."],
                    "IMPROVE": ["{{TREATMENT}} improves {{PROBLEM}}.", "{{TREATMENT}} cures {{PROBLEM}}."],
                    "REVEAL": ["{{TEST}} reveals {{PROBLEM}}."]}),
                             PipeParams('zero_shot_relation_extraction', 'setMultiLabel', False)]),
