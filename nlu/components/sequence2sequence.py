...
class Seq2Seq(SparkNLUComponent):
    ...
    def __init__(self, annotator_class='glove', language ='en', component_type='seq2seq', get_default=True, model = None, nlp_ref ='', nlu_ref ='',dataset=''):
        if 't5' in nlu_ref or 't5' in nlp_ref: annotator_class = 't5'
        if 'marian' in nlu_ref or 'marian' in nlp_ref: annotator_class = 'marian'
