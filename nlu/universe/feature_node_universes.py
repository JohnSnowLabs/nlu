...
class NLP_FEATURE_NODES():  # or Mode Node?
    ...
    nodes = {
        ...
        A.CHUNK2DOC: NlpFeatureNode(A.CHUNK2DOC, [F.CHUNK], [F.DOCUMENT_FROM_CHUNK]),
        ...
        A.DOC2CHUNK: NlpFeatureNode(A.DOC2CHUNK, [F.DOCUMENT], [F.CHUNK]),
        A.DOCUMENT_ASSEMBLER: NlpFeatureNode(A.DOCUMENT_ASSEMBLER, [E.RAW_TEXT], [F.DOCUMENT]),
        A.DOCUMENT_NORMALIZER: NlpFeatureNode(A.DOCUMENT_NORMALIZER, [F.DOCUMENT], [F.DOCUMENT]),
        ...
        A.NER_CONVERTER: NlpFeatureNode(A.NER_CONVERTER, [F.TOKEN, F.DOCUMENT, F.NAMED_ENTITY_IOB], [F.NAMED_ENTITY_CONVERTED]),
        ...
        A.TRAINABLE_NER_DL: NlpFeatureNode(A.TRAINABLE_NER_DL, [F.DOCUMENT, F.TOKEN, F.WORD_EMBEDDINGS], [F.NAMED_ENTITY_IOB]),
        ...
        A.TRAINABLE_SENTIMENT_DL: NlpFeatureNode(A.TRAINABLE_SENTIMENT_DL, [F.SENTENCE_EMBEDDINGS], [F.DOCUMENT_CLASSIFICATION]),
        ...
        A.SYMMETRIC_DELETE_SPELLCHECKER: NlpFeatureNode(A.SYMMETRIC_DELETE_SPELLCHECKER, [F.TOKEN], [F.TOKEN_SPELL_CHECKED]),
        ...
        A.LONGFORMER_FOR_TOKEN_CLASSIFICATION: NlpFeatureNode(A.LONGFORMER_FOR_TOKEN_CLASSIFICATION,
                                                              [F.DOCUMENT, F.TOKEN], [F.TOKEN_CLASSIFICATION]),
        A.MARIAN_TRANSFORMER: NlpFeatureNode(A.MARIAN_TRANSFORMER, [F.DOCUMENT], [F.DOCUMENT]),
        ...
        A.T5_TRANSFORMER: NlpFeatureNode(A.T5_TRANSFORMER, [F.DOCUMENT], [F.DOCUMENT]),
        ...
        A.BERT_FOR_SEQUENCE_CLASSIFICATION: None,  # TODO
        A.DISTIL_BERT_FOR_SEQUENCE_CLASSIFICATION: None,  # TODO

    }


@dataclass
class OCR_FEATURE_NODES():  # or Mode Node?
    ...
    A = OCR_NODE_IDS
    F = OCR_FEATURE
    VISUAL_DOCUMENT_CLASSIFIER = OcrFeatureNode(A.VISUAL_DOCUMENT_CLASSIFIER, [F.HOCR, F.FILE_PATH],
    ...
    IMAGE_TABLE_DETECTOR = OcrFeatureNode(A.IMAGE_TABLE_DETECTOR, [F.OCR_IMAGE, ],
                                          [F.OCR_TABLE])  # TODO REGION or TABLE??? IS IT THE SAME???
    IMAGE_TABLE_CELL_DETECTOR = OcrFeatureNode(A.IMAGE_TABLE_CELL_DETECTOR, [F.OCR_IMAGE, ],
                                               [F.OCR_TABLE_CELLS])  # TODO REGION or TABLE??? IS IT THE SAME???
    IMAGE_TABLE_CELL2TEXT_TABLE = OcrFeatureNode(A.IMAGE_TABLE_CELL2TEXT_TABLE, [F.OCR_IMAGE, F.OCR_TABLE_CELLS],
                                                 [F.OCR_TABLE])  # TODO OUPUT!! REGION or TABLE??? IS IT THE SAME???
    ...
    TEXT2PDF = OcrFeatureNode(A.TEXT2PDF, [F.OCR_POSITIONS, F.OCR_IMAGE, F.OCR_TEXT, F.FILE_PATH, F.BINARY_PDF],
                              [F.BINARY_PDF])
    ...
    # DICOM Processing
    DICOM2IMAGE = OcrFeatureNode(A.DICOM2IMAGE, [F.BINARY_DICOM, F.FILE_PATH],
    ...
    IMAGE_NOISE_SCORER = OcrFeatureNode(A.IMAGE_NOISE_SCORER, [F.OCR_IMAGE, F.OCR_REGION],
                                        [F.OCR_IMAGE])  # TODO WHAT IS REGION???? There is no schema for that
    ...
    IMAGE2TEXTPDF = OcrFeatureNode(A.IMAGE2TEXTPDF, [F.OCR_IMAGE, F.FILE_PATH, F.PAGE_NUM],
                                   [F.BINARY_PDF])  # TODO is output just normal binary PDF? Not 100% sure
    IMAGE2HOCR = OcrFeatureNode(A.IMAGE2HOCR, [F.OCR_IMAGE],
                                [F.HOCR])  # TODO is ouput HOCR format as in HOCR_DOCUMENT_ASSAMBLER???
    IMAGE_BRANDS2TEXT = OcrFeatureNode(A.IMAGE_BRANDS2TEXT, [F.OCR_IMAGE], [F.OCR_POSITIONS, F.TEXT,
                                                                            F.OCR_IMAGE])  # TODO what is the STRUCTURE of output image_brand ??? OCR_IE??

    # Other
    ## Find Positions of text in image
    POSITION_FINDER = OcrFeatureNode(A.POSITION_FINDER, [F.TEXT_ENTITY, F.OCR_PAGE_MATRIX],
                                     [F.OCR_POSITIONS])  # TODO COORDINATE==POSITION??
    ##  TODO Updates text at a position? I.e. Change the text at given corodinates BUT THEN why is output position???
    UPDATE_TEXT_POSITION = OcrFeatureNode(A.POSITION_FINDER, [F.OCR_POSITIONS, F.TEXT_ENTITY],
                                          [F.OCR_POSITIONS])  # TODO COORDINATE==POSITION??
    ## Cancer Document Test parser. Required Text of Header Field of something
    FOUNDATION_ONE_REPORT_PARSER = OcrFeatureNode(A.FOUNDATION_ONE_REPORT_PARSER, [F.OCR_TEXT, F.FILE_PATH],
...
@dataclass
class NLP_HC_FEATURE_NODES():  # Todo inherit EnumExtended  # or Mode Node?
    ...
    # HC Feature Nodes
    nodes = {
        A.ASSERTION_DL: NlpHcFeatureNode(A.ASSERTION_DL, [F.DOCUMENT, F.NAMED_ENTITY_CONVERTED, F.WORD_EMBEDDINGS], [H_F.ASSERTION]),
        # TODO chunk subtype?,
        A.TRAINABLE_ASSERTION_DL: NlpHcFeatureNode(A.TRAINABLE_ASSERTION_DL, [F.DOCUMENT, F.NAMED_ENTITY_CONVERTED, F.WORD_EMBEDDINGS],
                                                   [H_F.ASSERTION]),
        ...
        A.NER_DISAMBIGUATOR: NlpHcFeatureNode(A.NER_DISAMBIGUATOR, [F.CHUNK, F.SENTENCE_EMBEDDINGS],
                                              [H_F.DISAMBIGUATION]),
        A.RELATION_NER_CHUNKS_FILTERER: NlpHcFeatureNode(A.RELATION_NER_CHUNKS_FILTERER, [F.CHUNK, F.UNLABLED_DEPENDENCY],
        ...
        A.RELATION_EXTRACTION: NlpHcFeatureNode(A.RELATION_EXTRACTION,
                                                [F.WORD_EMBEDDINGS, F.POS, F.CHUNK, F.UNLABLED_DEPENDENCY], [H_F.RELATION]),
        ...
        A.SENTENCE_ENTITY_RESOLVER: NlpHcFeatureNode(A.SENTENCE_ENTITY_RESOLVER, [F.SENTENCE_EMBEDDINGS],
                                                     [H_F.ASSERTION]),
