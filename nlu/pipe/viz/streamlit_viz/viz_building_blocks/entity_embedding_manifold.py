...
class EntityEmbeddingManifoldStreamlitBlock():
    ...
    def viz_streamlit_entity_embed_manifold(
            ...
            sub_title: Optional[str] = "Apply any of the 11 `Manifold` or `Matrix Decomposition` algorithms to reduce the dimensionality of `Sentence Embeddings` to `1-D`, `2-D` and `3-D` ",
        ...
        #TODO
        """
        1. Require 1 NER model in pipe. NER always has Converter + Embed which we need for this
        2. Find NER model col and Word_embed col
        3. Create chunk_embedder from NER-Converter col and word_enedding col.
        """
        from nlu.pipe.viz.streamlit_viz.streamlit_utils_OS import StreamlitUtilsOS
        ...
        data = st.text_area('Enter N texts, seperated by new lines to visualize Sentence Embeddings for ',default_texts)
        # detect_sentence = False # TODO ITNEGRATE PARAM
        output_level = 'chunk' #if not detect_sentence else 'sentence'
        classifier_cols = []
        pipe = EntityManifoldUtils.insert_chunk_embedder_to_pipe_if_missing(pipe)
        ...
        if show_algo_select :
            ...
            algos = exp.multiselect(
                ...
                options=("TSNE", "ISOMAP",'LLE','Spectral Embedding','MDS','PCA','SVD aka LSA','DictionaryLearning','FactorAnalysis','FastICA','KernelPCA','LatentDirichletAllocation'),default=default_algos_to_apply,)

            # emb_components_usable = [e for e in Discoverer.get_components('embed',True, include_aliases=True) if 'chunk' not in e and 'sentence' in e]
            # # Todo, multi-classifiers excluded
            # classifier_components_usable = [e for e in Discoverer.get_components('classify',True, include_aliases=True) if 'xx' not in e and 'toxic' not in e and 'e2e' not in e ]
            # # Storage Ref extraction
            # loaded_embed_nlu_refs, loaded_storage_refs = StreamlitUtilsOS.extract_all_sentence_storage_refs_or_nlu_refs(e_coms)
            ...
            #     embed_algo_selection   = st.sidebar.multiselect("Pick additional Sentence Embeddings for the Dimension Reduction",options=emb_components_usable,default=loaded_embed_nlu_refs,key = key)
            #     embed_algo_selection=[embed_algo_selection[-1]]
            #
            #     exp = st.beta_expander("Pick additional Classifiers")
            #     class_algo_selection   = exp.multiselect("Pick additional Classifiers to load for coloring points",options=classifier_components_usable,default=loaded_classifier_nlu_refs,key = key)
            #     class_algo_selection=[class_algo_selection[-1]]
            #
            # else :
            #     exp = st.beta_expander("Pick additional Sentence Embeddings")
            #     embed_algo_selection   = exp.multiselect("Pick additional Sentence Embeddings for the Dimension Reduction",options=emb_components_usable,default=loaded_embed_nlu_refs,key = key)
            #     embed_algo_selection=[embed_algo_selection[-1]]
            #
            #     exp = st.beta_expander("Pick additional Classifiers")
            #     class_algo_selection   = exp.multiselect("Pick additional Classifiers to load for coloring points",options=classifier_components_usable,default=loaded_classifier_nlu_refs,key = key)
