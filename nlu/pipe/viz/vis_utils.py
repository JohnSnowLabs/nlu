...
class VizUtils():
    ...
    def infer_viz_type(pipe)->str:
        ...
        if pipe.has_licensed_compoents :
    ...
    @staticmethod
    def viz_OS(anno_res, pipe, viz_type,viz_colors):
        """Vizualize open source component"""
        if   viz_type == 'ner' : VizUtilsOS.viz_ner(anno_res, pipe,viz_colors)
        elif viz_type == 'dep' : VizUtilsOS.viz_dep(anno_res, pipe)
    ...
    @staticmethod
    def viz_HC(anno_res, pipe, viz_type,viz_colors):
        """Vizualize licensed component"""
        from nlu.pipe.viz.vis_utils_HC import VizUtilsHC
        if   viz_type == 'ner' : VizUtilsHC.viz_ner(anno_res, pipe,viz_colors)
...
#     'resolution' : [SentenceEntityResolverModel, ChunkEntityResolverModel] , #todo both work?
